...
**Появляется с PR `user-profile-block` (после его мержа в `main`): client-only профиль** (`@features/profile`, роут `/profile`, `ProfileAvatar` в обоих хедерах, включённый пункт `Profile` в `BottomNav`; имя хранится в `localStorage` под ключом `kinoshka:profile`, см. AGENTS.md, «Profile»). Когда в auth-ветках появится реальная сессия, заменяется только источник данных `useProfile()` — он начинает читать имя/логин из сессии BFF вместо `localStorage`; роут, страница, аватар и навигация переиспользуются как есть.

- [ ] В каждой auth-ветке (5.1-5.3): подключить `useProfile()` к сессии вместо `localStorage` (роут `/profile`, страница, `ProfileAvatar` и пункт `BottomNav` не переписываются).
- [ ] Заменить информационную подпись о локальном профиле на `/profile` реальной кнопкой входа/выхода.
- [ ] Решить судьбу локального имени из `kinoshka:profile`: мигрировать в серверный профиль или осознанно отбросить.
