...
export const mapDtoToMovieDetail = (doc: MovieDtoV14): MovieDetail => ({
  ...
  cast:
    doc.persons?.filter(isCast).map((person) => ({
      id: person.id,
      name: person.name ?? "",
  ...
  crew:
    doc.persons?.filter(isCrew).map((person) => ({
      id: person.id,
      name: person.name ?? "",
  ...
  budget:
    doc.budget?.value != null
      ? { value: doc.budget.value, currency: doc.budget.currency ?? "" }
      : undefined,
  feesWorld:
    doc.fees?.world?.value != null
      ? { value: doc.fees.world.value, currency: doc.fees.world.currency ?? "" }
      : undefined,
