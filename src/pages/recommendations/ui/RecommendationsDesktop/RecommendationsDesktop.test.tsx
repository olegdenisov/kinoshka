...
afterEach(() => {
  // RecommendationsDesktop рендерит Header, чей ThemeToggle применяет data-theme на
...
describe('RecommendationsDesktop — пустой список избранного', () => {
...
describe('RecommendationsDesktop — непустое избранное, успешный подбор', () => {
  it('рендерит карточки рекомендаций; исходящий запрос реально содержит id/genres.name/rating.kp правила', async () => {
    ...
    // Без кнопки-сердечка на этой странице (см. Technical Details плана) — Card получает
    // isFavorite/onToggleFavorite оба undefined, так что кнопка избранного не рендерится
    // (в отличие от нав-пункта "Favorites" в Header, который матчит /favorite/i тоже).
    expect(
...
describe('RecommendationsDesktop — все избранные id 404-нулись', () => {
...
describe('RecommendationsDesktop — каталог не вернул совпадений', () => {
...
describe('RecommendationsDesktop — Retry реально переинвалидирует кэш каталога, а не только избранного', () => {
