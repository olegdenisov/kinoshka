...
export const HomeMobile = () => {
  ...
  const sections = [
    { title: 'Popular this week', subtitle: 'Watching now', items: CATALOG.slice(0, 8) },
    { title: 'Trending series', subtitle: 'Binge-worthy', items: CATALOG.filter((m) => m.type === 'tv-series').concat(CATALOG.slice(0, 4)).slice(0, 8) },
    { title: 'Top anime', subtitle: 'Hand-picked', items: CATALOG.filter((m) => m.type === 'anime').concat(CATALOG.slice(4, 8)).slice(0, 8) },
    ...
    <div style={{ background: '#0F0D11', color: '#F2F0EF', minHeight: '100vh', paddingBottom: 80 }}>
      ...
      <section style={{ position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, transparent 50%, #0F0D11 100%)' }} />
        </div>

        <div style={{ position: 'relative', padding: '32px 20px 40px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{
            ...
            textTransform: 'uppercase', color: '#B8ADAB', alignSelf: 'flex-start',
          }}>
            <span style={{ width: 5, height: 5, borderRadius: 999, background: '#D18E5F', boxShadow: '0 0 0 3px rgba(209,142,95,0.18)' }} />
            <span>148,230 titles</span>
          </div>

          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 40, lineHeight: 1, letterSpacing: '-0.035em', fontWeight: 500, margin: 0 }}>
            ...
            {chips.map((c) => (
              <button
                ...
                style={{
                  ...
                  background: activeFilter === c.key ? 'rgba(209,142,95,0.15)' : 'rgba(24,22,27,0.6)',
