...
// Backslash экранируется ПЕРВЫМ, до '|': иначе входное значение вида '\|'
// (обратный слеш прямо перед вертикальной чертой) превратилось бы в '\\|' —
// в markdown это читается как экранированный '\', за которым следует уже
// НЕэкранированный '|', то есть таблица снова разъезжается (найдено CodeQL,
// "Incomplete string escaping or encoding", после первой версии этой функции).
