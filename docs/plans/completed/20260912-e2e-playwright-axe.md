...
- [x] все 6 smoke-сценариев из roadmap 2.5.5 покрыты и проходят: главная, поиск, фильтр, деталь+табы, favorites persist, theme toggle — перечитаны `e2e/home.spec.ts`, `e2e/search.spec.ts` (оба сценария: поиск + фильтр), `e2e/movie-detail.spec.ts`, `e2e/favorites.spec.ts`, `e2e/theme.spec.ts`; все 6 присутствуют и структурно соответствуют описанию из Task 3-6/9; прогон `pnpm exec playwright test --project=chromium` подтверждает: все 8 chromium-тестов зелёные
- [x] `/popular` и `/recommendations` тоже покрыты и проходят (Task 7, Task 8) — `e2e/popular.spec.ts` и `e2e/recommendations.spec.ts` подтверждены чтением и зелёным прогоном (в составе тех же 8 chromium-тестов)
- [x] `/favorites` покрыта как отдельная страница (не только тумблер избранного на `/`) — подтверждено чтением `e2e/favorites.spec.ts`: после `page.reload()` тест делает `page.goto('/favorites')` и проверяет наличие карточки по сохранённому `title` через `getByRole('link', { name: title, exact: true })`, плюс отдельный `checkA11y(page)` на `/favorites`
- [x] в каждом спеке есть `checkA11y` без critical violations — подтверждено грепом (`grep -c checkA11y`) по всем 8 файлам `e2e/*.spec.ts` + `e2e/mobile/smoke.spec.ts`: везде есть импорт и минимум один вызов; полный прогон всех chromium- и mobile-тестов не выявил ни одного assertion failure на `checkA11y` (единственные 3 провала — Mobile Safari, падают до `page.goto`, см. ниже)
- [x] mobile project существует и проходит хотя бы на `/`, `/search`, `/movie/:id` — `e2e/mobile/smoke.spec.ts` содержит по одному тесту на каждый из трёх роутов (перечитан целиком); **сама логика/селекторы валидированы** (см. Task 10) прогоном через Chromium с эмулированным mobile-viewport, все 3 теста прошли. Прогон через реальный project `Mobile Safari` на этой машине по-прежнему падает с тем же документированным в Task 10 `Protocol error (Page.overrideSetting): Unknown setting: PushAPIEnabled` (см. ниже) — известное локальное ограничение окружения (macOS 14.4/`mac14-arm64`, frozen WebKit-бинарник), не новая проблема
- [x] `pnpm exec playwright test` (все projects, без шардинга) проходит локально целиком против реального API — прогнан после свежего `make build-only`: **8 passed (chromium, 46.6s)**, **3 failed (Mobile Safari)** — все три с идентичным `Test timeout of 30000ms exceeded while setting up "page". Error: browserContext.newPage: Protocol error (Page.overrideSetting): Unknown setting: PushAPIEnabled`, падают ещё до первого `page.goto`, то есть до какого-либо тестового кода — в точности документированное в Task 10 известное локальное ограничение (frozen WebKit-бинарник на macOS 14.4/`mac14-arm64`, CI на `ubuntu-latest` ставит WebKit штатно, см. Task 11/Context), не новый регресс
- [x] `make check` (format-check + lint + build) и `make test` проходят без регрессий — `make test`: **82 файла / 694 теста, все зелёные** (то же число, что и в Task 2, регрессий нет); `make check` падает на шаге `format-check` (make останавливается на первом упавшем prerequisite: `check: format-check lint build`) — тот же документированный в Task 11 набор **9 pre-existing markdown-файлов** (`.revmux/profile.md`, `docs/backlog/play-trailer-inline.md`, `docs/plans/20260912-e2e-playwright-axe.md`, 5 файлов в `docs/plans/completed/`), ни один из них этим планом не тронут; `lint` и `build` (`tsc -b` + vite build) прогнаны отдельно (`make lint`, `make build`) и оба прошли чисто (exit 0) — новых нарушений сверх уже известных markdown-файлов не найдено
- [x] CI-конфиг синтаксически валиден (визуальная проверка YAML/`actionlint` при наличии) — повторно провалидирован `python3 -c "import yaml; yaml.safe_load(...)"`: парсится без ошибок, все 8 jobs на месте (`lint, typecheck, test, audit, size, knip, build, e2e`); `actionlint` по-прежнему недоступен в окружении (см. Task 11) — визуальный re-check подтверждает то же, что уже было зафиксировано в Task 11
...
- **Пересмотреть 4-way `matrix.shard` в `e2e`-job'е, если сьют не вырастет ощутимо.** Ревью (STEP 3 code-review, 2026-09-15) отметило, что 9 спек-файлов/11 тестов (46.6s серийно на одном chromium-воркере, см. Task 12) — вероятно, слишком мало для 4-machine шардинга: каждый шард independently повторяет checkout/`pnpm install`/`make e2e-install` (apt-пакеты WebKit)/`make build-only`/поднятие `vite preview`, и эта фиксированная стоимость сетапа, скорее всего, доминирует над временем самого прогона 2-3 тестов на шард — т.е. `--workers=4` в одном job'е (буквальная формулировка roadmap, от которой Task 11 сознательно отклонился, см. Technical Details) дал бы тот же уровень параллелизма без учетверения CI-минут/расхода `--with-deps`. Не применено сейчас: это разворачивало бы уже задокументированное архитектурное решение Task 11 (Solution Overview) без эмпирического подтверждения, что шардинг реально проблема (а не просто теоретически дороже) — оставлено как рекомендация на будущее, а не как немедленный фикс.
