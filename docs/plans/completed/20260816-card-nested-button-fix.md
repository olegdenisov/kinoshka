...
- `CardBtn.tsx` и `MobileCard.tsx`: убрать `e.preventDefault()` из обработчиков клика кнопок — он был нужен только для гашения нативной навигации нависшего `<a>`-предка, которого после рефакторинга больше нет. Изначально план предполагал оставить `e.stopPropagation()` без изменений (трогать его не входило в задачу этого плана) — **но фактически это решение было пересмотрено на review-фазе**: коммитом `fca8aff` `e.stopPropagation()` тоже был убран из обоих обработчиков, поскольку после перехода на stretched-link кнопки стали DOM-соседями `<Link>` заголовка, а не его потомками — клик по кнопке физически не может всплыть до ссылки, и `stopPropagation()` не имел никакого оставшегося эффекта (мёртвый код).
...
- [x] в `CardBtn.tsx` убрать `e.preventDefault()` из `onClick` (`e.stopPropagation()` по плану оставался без изменений на момент этой задачи — фактически удалён отдельно позже, на review-фазе, см. пункт ниже)
- [x] убрать `e.preventDefault()` из обработчика favorite-кнопки в `MobileCard.tsx` (он там есть — см. Context) (`e.stopPropagation()` по плану оставался без изменений на момент этой задачи — фактически удалён отдельно позже, на review-фазе, см. пункт ниже)
...
- [x] **Отклонение от плана (review-фаза, коммит `fca8aff`):** `e.stopPropagation()` в `CardBtn.tsx` и в обработчике favorite-кнопки `MobileCard.tsx` был дополнительно убран как мёртвый код — после DOM-перестройки на stretched-link (Task 1/2) кнопки стали соседями `<Link>` заголовка, а не его потомками, поэтому клик по кнопке больше никогда не всплывает до ссылки и `stopPropagation()` не давал никакого эффекта. Пункты выше и соответствующая строка Solution Overview изначально предписывали оставить `stopPropagation()` без изменений — это решение было пересмотрено позже, а не в рамках исходного Task 3.
