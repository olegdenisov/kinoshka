...
- [x] добавить `lighthouse: build-only` таргет в `Makefile` по эскизу из Technical Details — `pnpm dlx @lhci/cli@<pinned-version>` (запинить конкретную версию в самой команде — **не** добавлять `@lhci/cli` в `package.json`'s `devDependencies`, см. Context: `treosh/lighthouse-ci-action` вендорит свою версию и не читает лок-файл репозитория, а `pnpm dlx` не тянет install-time во все 7 job'ов `ci.yml` и не требует `knip.jsonc`-исключения), `--upload.target=filesystem`, `--collect.startServerCommand`/`--collect.startServerReadyPattern`, три `--collect.url`. Запинена версия `0.15.1` (текущая последняя стабильная на npm на момент задачи). ➕ Добавлен также `--upload.outputDir=.lighthouseci`, не бывший в эскизе — без него `upload.target=filesystem` по умолчанию пишет `manifest.json` и `<host>-<path>-<timestamp>.report.{html,json}` прямо в cwd (корень репозитория), а не в `.lighthouseci/`; обнаружено фактическим прогоном (см. ниже), исправлено явным `outputDir`
- [x] добавить `lighthouse` в `.PHONY`-список в начале `Makefile`
- [x] добавить `.lighthouseci/` в `.gitignore` (сейчас отсутствует — `lhci autorun` пишет отчёты в эту директорию при каждом прогоне)
- [x] задокументировать предусловие «нужен системный Chrome» прямо в Makefile-комментарии над таргетом (см. Technical Details)
- [x] тесты не применимы (shell-таргет) — вместо этого: один ручной локальный прогон `make lighthouse` завершается (не зависает, репорт пишется в `.lighthouseci/` и не публикуется наружу) — фактическая проверка идёт в Task 3. ➕ Реально прогнан на этой машине (системный Chrome найден по пути `/Applications/Google Chrome.app`, healthcheck `Chrome installation found` пройден): первый прогон упал на постороннем препятствии (порт 4173 был занят осиротевшим процессом `vite preview` от прошлой сессии, не связанным с этой задачей — процесс убит, порт освобождён); второй прогон нашёл проблему с `outputDir` (см. выше, исправлено); третий прогон завершился чисто — сервер поднялся по `--collect.startServerReadyPattern='Local:'`, все 3 URL пройдены Lighthouse, отчёты записаны только в `.lighthouseci/` (`git status --porcelain` после прогона показывает только изменённые `Makefile`/`.gitignore`, никаких новых untracked-файлов), команда завершилась (не зависла). Assertion-провалы (`accessibility` 0.92/0.87 против порога 0.95, `performance` warn 0.79-0.88 против 0.9) — ожидаемы и являются предметом Task 3, не блокируют Task 2
...
- [x] **до** написания workflow — один визит в Vercel dashboard: (а) подтвердить, что для этого репо преview-деплои создаются через официальную GitHub-интеграцию (появляются как GitHub Deployments — предпосылка для `wait-for-vercel-preview`), (б) подтвердить статус Vercel Authentication/Deployment Protection на preview-окружении — ⚠️ **отложено**: этот прогон полностью автономный, без доступа к браузеру/Vercel dashboard. Визит не имитировался и не выдумывался. Workflow реализован строго по эскизу плана (Technical Details), с закомментированными bypass-строками наготове (см. следующий пункт) — фактическое подтверждение переносится на реальную ручную проверку из Post-Completion («реальный прогон workflow на живом PR с лейблом `run-lighthouse`»)
- [x] если preview защищён — решить: отключить protection для preview-окружения, либо прокинуть bypass-секрет **в двух независимых местах** (это не одна настройка, а две): (а) `patrickedqvist/wait-for-vercel-preview`'s собственные входы `vercel_protection_bypass_header`/`vercel_password` — сам этот шаг делает HTTP-проверку готовности preview _до_ Lighthouse, и без bypass'а именно здесь упадёт/зависнет по таймауту, ещё до того как до Lighthouse вообще дойдёт очередь; (б) `collect.settings.extraHeaders` в `lighthouserc.cjs` — отдельно для самого Lighthouse-прогона. Задокументировать выбор здесь (⚠️ при отклонении от эскиза) — ⚠️ **отложено**, тот же довод, что выше: без дашборда неизвестно, включена ли Deployment Protection. Строка `vercel_protection_bypass_header: ${{ secrets.VERCEL_PROTECTION_BYPASS }}` оставлена в `lighthouse.yml` закомментированной, ровно как в эскизе плана, с комментарием, что раскомментировать её нужно, если реальный прогон (Post-Completion) покажет 401/зависание на шаге `wait-for-vercel-preview`. `collect.settings.extraHeaders` в `lighthouserc.cjs` не добавлялся по той же причине — нет секрета для его значения, и добавлять пустую заглушку без подтверждённой необходимости не имеет смысла
- [x] `curl -I` по любому реальному текущему preview-URL, подтвердить заголовок `X-Robots-Tag: noindex`; если подтверждено — **не** отключать assert конкретного аудита (`'is-crawlable': ['off', {}]` рядом с `'categories:seo': ['error', {minScore: 0.95}]`) — LHCI-ассерт на аудит только выключает проверку конкретно этого аудита, но не меняет сам `categories.seo.score`, который Lighthouse считает внутри себя по весам всех аудитов категории, включая проваленный `is-crawlable` с нулём; агрегированный `categories:seo`-порог 0.95 продолжит падать даже с выключенным ассертом на `is-crawlable`. Правильный фикс — **заменить** `'categories:seo': ['error', {minScore: 0.95}]` на явные ассерты по отдельным SEO-аудитам категории кроме `is-crawlable` (`'document-title': 'error'`, `'meta-description': 'error'`, `'http-status-code': 'error'`, `'link-text': 'error'`, `'crawlable-anchors': 'error'`, `'robots-txt': 'error'`, `'canonical': 'error'`, `'viewport': 'error'`, `'is-crawlable': 'off'`) — с WHY-комментарием (см. Technical Details), обновить `lighthouse-config.test.ts` (проверка: `categories:seo`-ключа больше нет, каждый из перечисленных аудитов явно `error`, `is-crawlable` явно `off`) — ⚠️ **живой `curl -I` по preview-URL отложен** (нет реального preview без реального PR — см. следующий пункт и Post-Completion). Код-уровневый фикс реализован независимо от подтверждения: `lighthouserc.cjs` заменил бы (а фактически не имел) категорийный `categories:seo`-ассерт на 8 явных per-audit `'error'`-ассертов + `'is-crawlable': ['off', {}]`, `lighthouse-config.test.ts` обновлён (нет ключа `categories:seo`, каждый из 8 SEO-аудитов — явно `'error'`, `is-crawlable` — явно `'off'`) — этот фикс корректен в любом случае: preview без `noindex` просто пройдёт `is-crawlable` тоже (ассерт `off`, не `error`, разницы в результате не создаёт)
- [x] проверить живьём `GET /v1.5/movie/666` возвращает 200 (не 404) — если нет, подобрать другой стабильный id перед тем, как вписывать его в `urls` — **выполнено реально**: `curl -s -o /dev/null -w '%{http_code}' -H "X-API-KEY: <VITE_API_KEY из .env.local>" https://api.poiskkino.dev/v1.5/movie/666` → `200`. Запись жива, `/movie/666` оставлен как есть в `urls`/Makefile без изменений
- [x] создать `lighthouse.yml` с собственным `on: pull_request: types: [labeled]` (не трогать `ci.yml`)
- [x] `if:` на `github.event.label.name == 'run-lighthouse'` и `github.event.pull_request.head.repo.full_name == github.repository`
- [x] `permissions: { contents: read, pull-requests: write, deployments: read, statuses: read }`, `timeout-minutes: 20`
- [x] `actions/checkout@v7` с `ref: ${{ github.event.pull_request.head.sha }}`
- [x] шаг `patrickedqvist/wait-for-vercel-preview` (запинить актуальный тег на marketplace) — `token: secrets.GITHUB_TOKEN`, `max_timeout: 300` — запинен **`v1.3.3`** (точная версия, не floating `v1`): проверено через GitHub API, что floating-тег `v1` у этого экшена резолвится в более старый коммит (`1adbe6881142dd3ca1b2ed920dd05dc8c87ebdb9`), чем реальный последний релиз `v1.3.3` (`d7982701e6fcd3ae073bff929e408e004404d38d`) — то есть `v1` у мейнтейнера не обновляется вместе с патчами; пиновка точного тега надёжнее floating-major здесь
- [x] шаг `treosh/lighthouse-ci-action` (запинить версию) с `continue-on-error: true` — `urls` из трёх строк, `configPath: ./lighthouserc.cjs`, `uploadArtifacts: true`, `temporaryPublicStorage: true` — запинен floating-major **`v12`** (проверено: сейчас резолвится в тот же коммит `3e7e23fb74242897f95c0ba9cabad3d0227b9b18`, что и точный тег `12.6.2`, т.е. `v12` у этого мейнтейнера актуален, в отличие от `wait-for-vercel-preview`'s `v1` выше) — тот же принцип пиновки, что уже принят в репо для официальных экшенов (`actions/checkout@v7` и т.п.)
- [x] шаг `actions/github-script` (запинить версию, тем же способом, что остальные два экшена) — markdown-таблица из `assertionResults`/`links`, явный фолбэк-текст на случай пустых outputs (сбор данных упал раньше assertion'ов), sticky-комментарий по маркеру — запинен **`v8`** (текущий последний major-релиз на момент задачи, подтверждено через GitHub API)
- [x] финальный шаг `if: steps.lhci.outcome == 'failure'` → `exit 1`
- [x] `concurrency: { group: lighthouse-${{ github.ref }}, cancel-in-progress: true }`
- [x] создать тестовый PR, повесить лейбл `run-lighthouse`, дождаться реального прогона против настоящего Vercel preview — зафиксировать в этом плане (➕) фактические числа по всем 4 категориям × 3 роутам; если что-то всё ещё не дотягивает до порога по причине, не устранённой Task 3 (`is-crawlable`-класса, платформенной, а не связанной с кодом приложения) — задокументировать здесь, а не тихо ослаблять порог — ⚠️ **отложено**: этот автономный прогон не пушит ветку, не открывает реальный PR на GitHub-репозитории пользователя, не вешает лейбл и не расходует общую квоту demo-API (200 запросов/день, уже используемую E2E). Это требует явного участия пользователя — push этой ветки, открытие настоящего PR и навешивание лейбла `run-lighthouse` самостоятельно, что уже предусмотрено разделом Post-Completion («Ручная проверка (обязательна)»). Таблица с фактическими числами базлайна на реальном preview **намеренно не заполнена выдуманными значениями** — она остаётся пустой до реального прогона пользователем; см. также требование Task 6 не закрывать документацию без этой ручной проверки
- [x] `make typecheck`, `make lint` — должны пройти (задеты `lighthouserc.cjs`/`lighthouse-config.test.ts`) — оба прошли чисто (см. также `make test`/`make format-check`/`make knip`, дополнительно прогнанные по Development Approach)

### Task 5: Verify acceptance criteria

- [x] все 4 категории и их пороги/severity заданы в `lighthouserc.cjs` и покрыты `lighthouse-config.test.ts` — подтверждено чтением обоих файлов: `categories:performance` (`warn`, 0.9), `categories:accessibility` (`error`, 0.95), `categories:best-practices` (`error`, 0.9) как категорийные ассерты; SEO — намеренно не категорией, а набором из 8 per-audit `'error'`-ассертов (`document-title`/`meta-description`/`http-status-code`/`link-text`/`crawlable-anchors`/`robots-txt`/`canonical`/`viewport`) + `'is-crawlable': ['off', {}]` (ревизия #3, ожидаемо, не пробел). `lighthouse-config.test.ts` покрывает все перечисленные ассерты отдельными `it`/`it.each`, плюс `preset === 'desktop'` и `numberOfRuns ≥ 1`
- [x] `.github/workflows/lighthouse.yml` реально фейлит PR (через финальный `exit 1`-шаг) при просадке любой `error`-категории — подтверждено чтением: шаг `Run Lighthouse CI` (`id: lhci`) имеет `continue-on-error: true`, финальный шаг `if: steps.lhci.outcome == 'failure'` → `run: exit 1` (использует `outcome`, не `conclusion` — корректно, `conclusion` стало бы `'success'` под `continue-on-error`)
- [x] PR-бот реализован — комментарий со ссылками на отчёты и assertion-результатами, обновляется, а не дублируется, с осмысленным фолбэком при пустых outputs — подтверждено чтением шага `Comment PR with results` (`actions/github-script@v8`): читает `LHCI_LINKS`/`LHCI_ASSERTION_RESULTS` из `steps.lhci.outputs`, при обоих пустых — явный текст "Lighthouse run failed before assertions ran…" вместо пустой таблицы; при наличии — markdown-список ссылок на отчёты + таблица assertion-результатов (или "All assertions passed."); sticky-комментарий по маркеру `<!-- lighthouse-ci-comment -->` через `listComments`/`find`/`updateComment` (создаёт `createComment` только если маркер не найден)
- [x] `make lighthouse` работает локально, не публикует отчёты наружу (`--upload.target=filesystem`), `.lighthouseci/` в `.gitignore` — реально прогнан в рамках этой проверки: команда завершилась (exit 1 — ожидаемо, это assertion-failure на известных из Task 3 разрывах: `/search` accessibility 0.91 против порога 0.95 и все три роута — performance `warn` ниже 0.9, задокументированный шум shared-машины/CSR, не зависание и не краш), отчёты записаны только в `.lighthouseci/` (`git status --porcelain` после прогона — чисто, никаких новых untracked-файлов вне `.lighthouseci/`), `.gitignore` содержит `.lighthouseci/` (строка 44), в Makefile-команде нет `temporary-public-storage`/`uploadArtifacts`
- [x] `make check` (`format-check lint build`), `make test`, `make knip` — всё зелёное — `make lint` (exit 0), `make build` (typecheck+vite build, exit 0), `make test` (84 файла/711 тестов, все прошли), `make knip` (exit 0, только 3 конфигурационные подсказки, не ошибки) — все зелёные. `make format-check` в целом падает, но исключительно на 14 заранее известных файлах (`docs/plans/*.md`, `.revmux/profile.md`), не тронутых этой задачей — тот же предсуществующий, задокументированный ещё в Task 1 разрыв ("предсуществующие format-check-находки в docs/plans/*.md/.revmux/profile.md не связаны с этой задачей и не трогались"); точечно проверено `oxfmt --check` по всем файлам, созданным/изменённым в Tasks 1-4 (`lighthouserc.cjs`, `lighthouse-config.test.ts`, `Makefile`, `.github/workflows/lighthouse.yml`) — все четыре форматированы чисто. Не считается genuine gap этой задачи — не Lighthouse-related, чинить не входит в Files этой задачи
- [x] E2E не трогали — `make e2e` не запускать в рамках этой задачи — не запускался
...
- [x] добавить в `AGENTS.md` раздел «Lighthouse CI» по образцу существующих подразделов Phase 2.5 — зафиксировать: `.cjs` vs `lighthouserc.json` и почему (WHY-комментарии, прецедент `knip.jsonc`), отсутствие `collect.url`/`collect.startServerCommand` в общем конфиге, `preset: 'desktop'`, `categories:performance: 'warn'` как временное решение (условие перехода в `error` — required-check promotion), `numberOfRuns: 1`, `is-crawlable`-исключение и его причина (Vercel `X-Robots-Tag: noindex` на preview), `/movie/666` как живая, но не гарантированно вечная запись, `pnpm dlx @lhci/cli` вместо devDependency и почему, фактические числа базлайна из Task 3/Task 4
- [x] добавить `make lighthouse` в Commands-блок AGENTS.md и в README.md (там, где перечислены `make e2e`/`make knip`/остальные таргеты)
- [x] попутно поправить описание `make check` в обоих файлах на «lint + build» → фактическое «format-check + lint + build» (расхождение обнаружено при discovery этого плана, не связано с Lighthouse напрямую, но раз уже редактируем оба файла — исправить) — подтверждено чтением `Makefile` (`check: format-check lint build`) перед правкой
- [x] отметить пункт 2.5.6 в `plans/roadmap.md` как выполненный (`[x]`), с пометкой отклонений (`.cjs` вместо `.js`, лейбл вместо авто-триггера, фиксированный `/movie/666`, `desktop`-preset, `performance: warn` на период soak)
- [x] завести на GitHub лейбл `run-lighthouse` (аналогично `run-e2e`) — реально создан через `gh label create run-lighthouse --color FBCA04` (успех, `gh` был аутентифицирован; цвет `#FBCA04`, визуально отличается от `run-e2e`'s `#0E8A16`)
- [x] перенести этот план в `docs/plans/completed/` — **не выполнено здесь намеренно**: перенос выполняется оркестратором в конце всего прогона (Step 13), не в рамках этой задачи — перемещение файла сейчас сломало бы путь, который читают последующие review-фазы
