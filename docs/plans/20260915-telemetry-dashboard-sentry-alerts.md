...
3. **Error-rate alert реализован как Metric Alert (count-based), а не Issue Alert.** Изначально обсуждалась формулировка "issue замечен ≥10 уникальными пользователями за 60 минут" (Issue Alert). От неё отказались по двум причинам, найденным во втором раунде ревью: (а) `sentry alert issues create --condition` целится в новый workflow-native JSON-формат, чей `type` для "unique user frequency" не задокументирован и не подтверждён живым пробным вызовом — это был бы неоценённый исследовательский риск прямо в середине задачи; (б) порог "≥10 уникальных пользователей" на низком трафике портфолио-проекта менее вероятно наберётся, чем count-based альтернатива — один и тот же баг в одной пользовательской сессии обычно кидает одну и ту же ошибку по нескольку раз, так что `count() событий ≥ 10 за 60 минут` на практике более достижим, а не менее actionable. Metric Alert к тому же переиспользует ровно тот же builder/idempotency-путь, что и LCP-алерт (один код вместо двух).
...
- **Error rate alert (Metric Alert, count-based — см. "Принятые решения" про отказ от Issue Alert):** `sentry alert metrics create <org> --name '<name>' --query 'event.type:error' --aggregate 'count()' --dataset errors --time-window 60 --project <project> --trigger '[{"alertThreshold":<ERROR_ALERT_COUNT_THRESHOLD>,"actions":[{"id":"sentry.mail.actions.NotifyEmailAction","targetType":"Team","targetIdentifier":<teamId>}]}]'` — форма флагов и `--trigger` дословно из живого `sentry alert metrics create --help`.
...
- **Dashboard "Kinoshka Telemetry"** — 6-колоночная сетка (ширины виджетов по типу зафиксированы в `sentry dashboard widget add --help`: `big_number` 2×1, `line`/`area`/`bar` 3×2, `table` 6×2 — сумма ширин в строке должна быть 6, иначе CLI молча авто-разложит иначе): строка из 3 `big_number` (error count `--dataset errors --query count`, issues count, throughput) + `line`-график P75 LCP (`--dataset transactions --query p75:measurements.lcp`) + `table` топ issues (full-width, `--dataset issue`). **INP/CLS как виджеты дашборда — датасет требует живой проверки после того, как в проекте реально появятся ingested-данные** (в SDK v8+ INP, а в v9/v10 и CLS — standalone span, не measurement на транзакции; вероятный датасет — `spans`, не `transactions`, но не предполагается заранее). Roadmap просит алерт только на LCP — на INP/CLS алертов нет, только видимость на дашборде.
- **Idempotency** — для alert rules и дашборда: exists-check по имени в соответствующем list-выводе (`--fresh`, см. выше); если существует — пропуск создания, предупреждение в stdout. **Для виджетов** — если дашборд уже существует (то есть его виджеты, скорее всего, уже добавлены раньше этим же скриптом), скрипт **не трогает виджеты вообще** (ни создаёт заново, ни диффует) — обновление состава виджетов при существующем дашборде осознанно не автоматизируется в этом плане (ручной путь — `sentry dashboard widget edit`/`widget delete`), чтобы не городить diff-логику ради одноразового provisioning.
...
- [ ] `sentry-telemetry.config.ts`: `ERROR_ALERT_COUNT_THRESHOLD = 10`, `ERROR_ALERT_WINDOW_MINUTES = 60`, `LCP_P75_THRESHOLD_MS = 2500`, `LCP_ALERT_WINDOW_MINUTES = 15`
...
- [ ] добавить в `AGENTS.md` новый раздел "Telemetry дашборд (Sentry)" — задокументировать: `tracesSampleRate=0.2` и его trade-off, решение про `tracePropagationTargets`, пороги error-rate/LCP-алертов (и почему error-rate — count-based Metric Alert, а не Issue Alert), состав дашборда, idempotency-подход provisioning-скрипта, `.mcp.json`/Sentry MCP, порядок инициализации через `sentry-bootstrap.ts`, ссылку на `docs/telemetry-runbook.md`
- [ ] отметить `[x]` в `plans/roadmap.md` под `2.5.7`, с заметкой про отклонение от буквальной формулировки: Web Vitals переехали в Sentry, добавлен Sentry MCP сверх формулировки, error-rate реализован как count-based Metric Alert (не Issue Alert по уникальным пользователям), третий пункт ("конверсия по flows") закрыт раннбуком/ручной настройкой Plausible UI
