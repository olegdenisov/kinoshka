...
- [x] В `.env.local` добавить `VITE_SENTRY_DSN=`, `SENTRY_AUTH_TOKEN=`, `SENTRY_ORG=`,
      `SENTRY_PROJECT=` с плейсхолдер-значениями (или пустыми — по образцу существующих строк).
- [x] В `.env.example` — те же четыре переменные с явными плейсхолдерами (`<...>`), по образцу
      уже существующих строк.
- [x] Тестов не требует (статические файлы конфигурации, не код).
