...
- [ ] добавить `TrendingIcon` (инлайн SVG, сигнатура как у соседних `Icon`-компонентов — `size`) — растущий график/стрелка вверх, консистентно со стилем существующих иконок (`stroke='currentColor'`, минималистичные пути).
- [ ] отдельный тест не требуется (по прецеденту — остальные иконки в `Icon.tsx` не покрыты юнит-тестами индивидуально; косвенно проверяется тестами `BottomNav` в Task 8).
