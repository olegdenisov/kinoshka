...
- [ ] `grep -rE "#[0-9A-Fa-f]{3,8}|rgba?\(" --include='*.module.css' src` — построить полный список, свериться с шестью файлами выше
- [ ] `BottomNav.module.css:7` (`rgba(15,13,17,0.92)`) — UI-chrome фон навбара → заменить на `var(--bg-glass-heavy)` (или ближайший подходящий токен)
- [ ] `Card.module.css:96` (`color: rgba(242,240,239,0.7)`) — UI-chrome текст → заменить на `var(--text-secondary)` (или ближайший подходящий токен)
- [ ] `HeroSection.module.css:68,185`, `MovieHero.module.css:30-31`, `MobileCard.module.css:26,47` — определить для каждого: это скрим/оверлей над произвольным фото (постер/backdrop), нужный для читаемости независимо от темы → оставить хардкод с коротким комментарием `/* намеренно не тонируется — скрим над постером/backdrop */`, ИЛИ это чистый UI-фон → тонировать. Решение принимается визуально (открыть компонент в браузере)
- [ ] `Poster.module.css` — `rgba(255,255,255,...)` инсет-хайлайты и `rgba(242,240,239,...)` текст поверх сгенерированного (oklch-hue) плейсхолдера: по умолчанию оставить как намеренную консистентную стилизацию «постера» (не зависит от темы приложения, как и сам постер), задокументировать решение комментарием в CSS
- [ ] Тесты не требуются (визуальный CSS-рефакторинг), кроме случаев, если существующий тест где-то assert'ит инлайн-цвет (проверить аналогично находке в Task 11 — по этим файлам ассертов на стиль не найдено на момент планирования, но проверить `grep -rn "toHaveStyle\|\.style\." src/**/*.test.tsx` по затронутым компонентам)
- [ ] Визуально проверить каждый затронутый компонент в light и dark
- [ ] run `make test` — не должно быть регрессий перед Task 9
...
- [ ] Перенести компонент в директорию по стандартному паттерну проекта (`Component.tsx` + `index.tsx`-реэкспорт), импорт в `FavoritesPage.tsx` не меняется (без расширения, резолвится на директорию)
- [ ] Перенести все `style={{...}}` в `FavoritesMobile.module.css`, хардкод-хексы (`#F2F0EF`, `#0F0D11`) → `var(--text-primary)`/`var(--bg-primary)`
- [ ] Динамические/вычисляемые значения (если есть) оставить инлайн `style`
- [ ] Визуально сверить рендер до/после в dark-режиме (пиксель-в-пиксель идентично)
- [ ] `FavoritesMobile.test.tsx` — прогнать без изменений содержимого, должен остаться зелёным
- [ ] Визуально проверить light-режим
- [ ] run tests — должны пройти перед Task 10
...
- [ ] Пройти чек-лист 2.2 из `plans/roadmap.md` построчно и подтвердить каждый пункт
- [ ] Toggle работает на всех страницах (`/`, `/search`, `/movie/:id`, `/favorites`) в обоих Header (desktop resize < 720px → mobile)
- [ ] Persist: тема переживает reload страницы
- [ ] Нет FOUC: hard refresh с сохранённым в localStorage `"light"` (JSON-обёрнутое значение) на ОС с тёмной темой — не мигает тёмным перед светлым (и наоборот)
- [ ] `system` работает как начальное значение при первом визите (очистить localStorage, сравнить с ОС-настройкой)
- [ ] Cross-tab: смена темы в одной вкладке отражается в другой (через тот же `createStorageSlot`)
- [ ] Нативные контролы (`YearRangeSlider`, скроллбары) визуально соответствуют теме (`color-scheme` из Task 1 применился)
- [ ] `make check` (lint + build) — зелёный
- [ ] `make test` — весь сьют зелёный
- [ ] `make coverage` — сравнить с уровнем до задачи, не хуже
