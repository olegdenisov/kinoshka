...
- [x] Проверить, что все требования из Обзора реализованы (и поле поиска, и
...
- [x] Проверить граничные случаи (пустой запрос, тип не выбран, оба
      заданы, задан только тип) — покрыты тестами в
      `HeroSection.test.tsx` (Task 1), все проходят
- [x] Проверить, что сайдбар `/search` синхронизируется с типом из
      hero-навигации в browse-режиме, и корректно игнорирует его в
      text-search режиме (Task 2) — подтверждено тестами в
      `SearchDesktop.test.tsx` (Task 2), все проходят
- [x] Прогнать полный набор тестов: `make test` — 39 test files, 383 tests
      passed
- [x] Прогнать `make lint` и `make typecheck` — оба прошли без ошибок
- [x] Подтвердить отсутствие e2e-сьюта (шаг пропущен, уже отмечено в
      Стратегии тестирования) — подтверждено: нет Playwright/Cypress в
      `package.json`, нет каталогов `e2e`
