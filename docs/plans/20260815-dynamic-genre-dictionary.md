...
- [x] убедиться, что справочник жанров загружается из живого API и рендерится и в `SearchSidebar` (desktop), и в `SearchMobile`, без Suspense-фолбэка/мигания (первый рендер сразу со статическим шорт-листом) — подтверждено автоматизированно: `useGenreDictionary.test.tsx` («первый рендер сразу отдаёт статический фолбэк, без ожидания сети», «после успешного фонового фетча перерисовывается с данными из API») и `GenreSelector.test.tsx` («после подгрузки справочника (MSW) список расширяется реальными данными»); обе точки использования (`SearchSidebar`, `SearchMobile`) рендерят один и тот же `GenreSelector`, синхронный хук без `AsyncBoundary`/`use()`
- [x] проверить кэширование в localStorage: перезагрузить страницу — справочник рендерится из кэша мгновенно, фонового обновления не происходит в рамках 7-дневного TTL — подтверждено тестом `useGenreDictionary.test.tsx` → «свежий кэш — рендерится сразу из кэша, фонового запроса не происходит» (счётчик вызовов MSW-хендлера остаётся 0 при повторных ре-рендерах)
- [x] проверить, что жанр, выбранный по deep-link и отсутствующий в шорт-листе, отрисован и подсвечен в `GenreSelector` без ручного «Показать все» — покрыто тестом `GenreSelector.test.tsx` → «жанр из selected, не входящий в шорт-лист, тоже отрисован и подсвечен как активный» (`аниме`, `aria-pressed="true"`, без клика по «Показать все»)
- [x] проверить, что выбор жанра по-прежнему корректно фильтрует результаты поиска (сетевой запрос содержит правильное значение `genres.name`) — подтверждено на уровне модульных тестов сквозной цепочки: `GenreSelector` → `onToggle` вызывается с сырым RU-названием (`GenreSelector.test.tsx`, «выбор чипа вызывает onToggle»), `filtersToParams.test.ts` → RU-названия жанров проходят в `params['genres.name']` без изменений (включая немаппленные) — тот же путь, что и раньше, конструктивно гарантирован уже тем, что каноническое значение фильтра больше не проходит через отбрасывающий неизвестные значения `toApiGenre`
- [x] проверить, что немаппленные жанры справочника (без английского лейбла) рендерятся с русским названием в качестве лейбла чипа и по-прежнему корректно фильтруют — `genreMap.test.ts` → «неизвестное RU-название возвращается как есть (фолбэк)» (`аниме` → `аниме`), визуально то же самое подтверждает `GenreSelector.test.tsx` (чип `аниме` отрисован под тем же именем); фильтрация не завязана на наличие EN-лейбла — `filtersToParams` использует сырое RU-значение
- [x] проверить сценарий недоступного эндпоинта справочника (замокать 403/500): фильтр жанров остаётся рабочим на статическом шорт-листе, консоль не заваливает повторными запросами/React-варнингами про нестабильный `getSnapshot` — `useGenreDictionary.test.tsx` → «неудачный фоновый фетч не приводит к повторным запросам при последующих ре-рендерах в пределах cooldown» (ровно 1 запрос на 500-ошибку при нескольких ре-рендерах); полный прогон `make test`/`make coverage` не выдал ни одного React-варнинга/`getSnapshot`-предупреждения в консоли (`getSnapshot`-баг устранён и покрыт тестами ещё в Task 1)
- [x] прогнать полный набор тестов: `make test` — 43 test files / 411 tests passed
- [x] прогнать `make check` (lint + build) — подтвердить отсутствие регрессий по линту/типам — `oxlint` и `tsc -b && vite build` прошли чисто; `make check` целиком падает на `format-check` (`oxfmt --check`) из-за форматирования 4 markdown-файлов (`AGENTS.md`, 3 файла в `docs/plans/`) — подтверждено воспроизведением на чистом рабочем дереве без единой правки этого плана (`git stash` показал тот же сбой), т.е. это предсуществующее состояние репозитория, не регрессия этой работы
- [x] проверить покрытие тестами новых модулей на уровне остального проекта (`make coverage`, выборочно) — новые модули полностью покрыты (`genreMap.ts`, `filtersToParams.ts`, `genre.ts`, `genreDictionaryCache.ts`, `useGenreDictionary.ts` не попали в отчёт — репортер v8 скрывает файлы со 100% по всем метрикам) либо на уровне/выше среднего по проекту (`GenreSelector.tsx` 93.61% stmts против 86.37% в среднем по проекту; `getGenreDictionary.ts` 83.33% stmts — сопоставимо с соседними API-обёртками вроде `getMovies.ts`/`getMovieDetail.ts`, 83.33%)
