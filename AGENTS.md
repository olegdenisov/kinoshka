...
- **Release tagging:** the release string is built exactly once, in `vite.config.ts`, as `` `kinoshka@${version}+${gitSha}` `` — `version` read directly from `package.json` via `fs.readFileSync` (not `process.env.npm_package_version`, which isn't reliably set across `make build`/`make build-only`/CI), `gitSha` from `execSync('git rev-parse --short HEAD')` with a `try/catch` fallback to `'unknown'` so a build without `.git` available doesn't fail. The same string is passed both to the browser SDK (via a `define: { __APP_RELEASE__: JSON.stringify(release) }` constant, declared ambient in `src/vite-env.d.ts` and read by `initSentry()`) and to `sentryVitePlugin({ release: { name: release } })`, so the SDK and the source-map upload always tag the same release — otherwise the plugin would default to the bare git SHA and uploaded source maps wouldn't match the release recorded on incoming events.
